...
from .views import RegisterView, ActivateView, LoginView, CustomTokenRefreshView, LogoutView, PasswordResetView, PasswordConfirmView, activation_success

urlpatterns = [
    path('register/', RegisterView.as_view(), name='register'),
    path('activate/<uidb64>/<token>/', ActivateView.as_view(), name='activate'),
    path('token/refresh/', CustomTokenRefreshView.as_view(), name='token-refresh'),
    path('activated/', activation_success, name='activation_success'),
